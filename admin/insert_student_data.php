<?php 
                  ...
                  if (isset($_POST['submit'])) {
                      ...
                      $checksql  = "SELECT * FROM student_info WHERE status = '$statu' ";
                      $checkquery = $db->query($checksql);
                      $checkresult = mysqli_fetch_assoc($checkquery);
                      if (mysqli_num_rows($checkquery) != 0) {
                        $errors[] = 'You Updated your details before!';
                      ...
                      $required = array(
                        ...
                        if (!empty($errors)) {
                          ...
                          $statusc = 1;
                          $sql = "INSERT INTO `student_info` ( st_mobile, st_date_of_birth , st_father_name ,  st_father_mobile, st_mother_name ,  st_mother_mobile ,  st_lga, st_state, st_address, st_stream , st_date_of_admission, status); VALUES ( ?,?,?,?,?,?,?,?,?,?,?,?);";
                           // $query = $db->query($sql);
                           // if (!$query) {
                          ...
                          echo 'SQL Error'. mysqli_connect_error($db);die();
                            ...
                            mysqli_stmt_bind_param($stmt, 'ssssssssssss',  $st_mobile,$st_date_of_birth , $st_father_name ,  $st_father_mobile, $st_mother_name ,$st_mother_mobile ,  $st_lga_id,$st_state_id, $st_address,
                              $st_stream_id , $st_date_of_admission, $statusc );
