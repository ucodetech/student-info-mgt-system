            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-md-12">
                        <div class="card">
                            <div class="header">
                                <h4 class="title"><?=((isset($_GET['edit']))? 'Edit Profile': 'Admin Profile');?></h4>
                            </div>
                            <div class="content">
                                <?php
 if (isset($_GET['edit'])) {
    ...
    $username = ((isset($_POST['username']) && !empty($_POST['username']))?sanitize($_POST['username']): $adminf['username']);
        ...
        if ($_POST) {
            ...
            if (!empty($errors)) {
                ...
                if (isset($_GET['edit'])) {
                    $updateSql = "UPDATE admin SET username = '$username', full_name = '$full_name', email = '$email' WHERE id = '$edit_id' ";
                        $updateResult = $db->query($updateSql);
                }
                if ($updateResult) {
                  $_SESSION['success_flash'] = 'Admin Updated';                   
   ...
   ?>
  <form method="post" action="admin.php?<?=((isset($_GET['edit']))?'edit='.$edit_id : '');?>" enctype="multipart/form-data">
      <div class="row">
    ...
    <?php }else{
        ...
        $adminQuery =$db->query("SELECT * FROM admin WHERE id = '$user_id'");
            ?>
                 <div class="content table-responsive table-full-width">
                   ...
                   <tbody>
                     <?php while($use = mysqli_fetch_assoc($adminQuery)) : ?>
                     <tr>
                       <td>
      ...
      <div class="col-md-3 form2">
         <?php 
              ...
              if (mysqli_num_rows($query) > 0) {
                while ($row = mysqli_fetch_assoc($query)) {
                  ...
                  $adid = $row['id'];
                  $sqlim = "SELECT * FROM `admintprofile` WHERE admin_id = '$adid' ";
                  $queryim = $db->query($sqlim);
                  if (mysqli_num_rows($queryim) > 0) {
                    while ($rowimg = mysqli_fetch_assoc($queryim)) {
                      
                        if ($rowimg['status'] == 0) {
                          echo "<img src='profile/profile".$user_id.".jpg?'".mt_rand()." class=' align-self-start mr-3' alt='Admin Profile' width='210' height='200'>
               ";
                        }else{
                          ?>
           ...
           ?>
              <?php 
                ...
                $sqlquery  = $db->query("SELECT * FROM admin WHERE id = '$user_id' ");
                $rowd = mysqli_fetch_assoc($sqlquery);
                

               ?>
         <hr>
         <span class="name"> Name: <?=$rowd['full_name'];?></span><br>
        ...
        <div class="col-md-8 form">
            ...
            <div class="content">
          <div class="container-fluid">
            <h4 class="text-info text-center">Upload Your Picture (<span class="text-danger"><?=$user_data['permissions'];?> </span>)</h4><hr>
                <div class="container">
                  <div class="row">
